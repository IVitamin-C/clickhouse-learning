...
## 前言
本文主要针对实时场景，使用测试数据，模拟业务场景，简单介绍了物化视图的使用，目前这些案例已用于生产环境，表现良好。
clickhouse在单表性能上表现出非常强大的性能优势，但是在join场景上，表现平平，所以在使用上，尽可能的宽表化，但是也存在不同行为日志不同上报方式，上报途径的问题。
本文在后半部分介绍了在一些场景下，如何另辟蹊径，以宽表建设的方式避免join。
通过这篇文章，你将了解到以下内容:
1.物化视图如何创建，如何增加维度和指标
2.字典如何创建，如何结合物化视图实现增维
3.在多日志上报时，如何简单高效的将协议整合成宽表，避免join问题，提高查询效率和使用便捷性。
